...
int main() {
    ...
    // функциональный объект (mult()) применяется к каждому элементу массива с помощью алгоритма
    // for_each().
    struct mult {
    ...
    // Будем получать сведения о состоянии объекта. Объекту ar присваивается возвращаемый
    // алгоритмом for_each функтор Sum,
    ...
    //
    // Существуют встроенные функции (accumulate и partial_sum, заголовок <numeric>) реализующие
    ...
    struct Prd {
        ...
        // Перегрузка операции ()
        bool operator()(const int& v) { return v > my_cnt; }
    };
...
////
////// ПРИМЕР 3
////// Дан массив из 20 элементов. Произвести следующие операции, используя алгоритмы и
...
//  // Cумма элементов
//  //=============================================
//  for_each(first, last, [&sum](int el){ sum += el; });
...
//  //=============================================
//  sum = count_if(first, last, [](int &el){return el > 5;});
//  cout << "\nКоличество элементов > 5: " << sum << endl;
//  //=============================================
